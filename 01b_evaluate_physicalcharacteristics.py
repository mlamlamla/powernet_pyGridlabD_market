...
# Get estimates for part of the year
start = pd.Timestamp(2016,9,12)
end = pd.Timestamp(2016,9,19)

# Substitute missing values with which data
...
################
#
# Calculate beta and gamma for chosen week
#
################
...
################
#
# Add average of alternative period if values are missing
#
################

df_settings = pd.read_csv(folder+'/settings/HVAC_settings_'+str(start).split(' ')[0]+'_'+str(end).split(' ')[0]+'.csv',index_col=[0])
df_settings_backup = pd.read_csv(folder+'/settings/HVAC_settings_'+str(start_backup).split(' ')[0]+'_'+str(end_backup).split(' ')[0]+'.csv',index_col=[0])

for ind in df_settings.index:
	if (df_settings['gamma_cool'].loc[ind] <= 0.0):
		df_settings.at[ind,'gamma_cool'] = df_settings_backup.at[ind,'gamma_cool']
		df_settings.at[ind,'P_cool'] = df_settings_backup.at[ind,'P_cool']
	if (df_settings['heating_system'].loc[ind] != 'GAS') and (df_settings['gamma_heat'].loc[ind] <= 0.0):
		df_settings.at[ind,'gamma_heat'] = df_settings_backup.at[ind,'gamma_heat']
		df_settings.at[ind,'P_heat'] = df_settings_backup.at[ind,'P_heat']

df_settings.to_csv(folder+'/settings/HVAC_settings_'+str(start).split(' ')[0]+'_'+str(end).split(' ')[0]+'_mod.csv')